...
`
	...
	// Install Certbot for SSL certificates (with both nginx and apache plugins)
	output.WriteString("\n=== Installing Certbot for SSL ===\n")
	certbotOut, _, certbotErr := e.RunCommandWithExitCode(ctx, "apt-get", "install", "-y", "-qq", "certbot", "python3-certbot-nginx", "python3-certbot-apache")
	output.WriteString(certbotOut)
	if certbotErr != nil {
		output.WriteString(fmt.Sprintf("Warning: failed to install certbot: %v\n", certbotErr))
	} else {
		output.WriteString("Certbot installed (SSL certificates ready)\n")
	}
	...
	if _, err := os.Stat("/usr/bin/node"); os.IsNotExist(err) {
		...
		e.RunCommandWithExitCode(ctx, "bash", "-c", "curl -fsSL https://deb.nodesource.com/setup_22.x | bash -")
		nodeOut, _, nodeErr := e.RunCommandWithExitCode(ctx, "apt-get", "install", "-y", "-qq", "nodejs")
		output.WriteString(nodeOut)
		if nodeErr != nil {
			output.WriteString(fmt.Sprintf("Warning: failed to install Node.js: %v\n", nodeErr))
		} else {
			// Update npm to latest
			e.RunCommandWithExitCode(ctx, "npm", "install", "-g", "npm@latest")
			nodeVersion, _, _ := e.RunCommandWithExitCode(ctx, "node", "-v")
			npmVersion, _, _ := e.RunCommandWithExitCode(ctx, "npm", "-v")
			output.WriteString(fmt.Sprintf("Node.js %s and npm %s installed\n", strings.TrimSpace(nodeVersion), strings.TrimSpace(npmVersion)))
		}
	} else {
		nodeVersion, _, _ := e.RunCommandWithExitCode(ctx, "node", "-v")
		output.WriteString(fmt.Sprintf("Node.js already installed: %s\n", strings.TrimSpace(nodeVersion)))
	}
	...
	output.WriteString("\n=== Installing WP-CLI ===\n")
	if _, err := os.Stat("/usr/local/bin/wp"); os.IsNotExist(err) {
		// Download WP-CLI
		e.RunCommandWithExitCode(ctx, "bash", "-c", "curl -sS https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar -o /usr/local/bin/wp")
		e.RunCommandWithExitCode(ctx, "chmod", "+x", "/usr/local/bin/wp")
		if _, err := os.Stat("/usr/local/bin/wp"); err == nil {
			wpVersion, _, _ := e.RunCommandWithExitCode(ctx, "wp", "--version", "--allow-root")
			output.WriteString(fmt.Sprintf("WP-CLI %s installed\n", strings.TrimSpace(wpVersion)))
		} else {
			output.WriteString("Warning: failed to install WP-CLI\n")
		}
	} else {
		wpVersion, _, _ := e.RunCommandWithExitCode(ctx, "wp", "--version", "--allow-root")
		output.WriteString(fmt.Sprintf("WP-CLI already installed: %s\n", strings.TrimSpace(wpVersion)))
	}
...
func (e *Executor) handleProvisionMariaDB(ctx context.Context, payload json.RawMessage) comm.JobResult {
	var p struct {
		...
		Force          bool   `json:"force"` // If true, re-generate credentials even if they exist
	}
	json.Unmarshal(payload, &p)
	...
	log.Info().Int64("step_id", p.StepID).Str("version", p.Version).Bool("force", p.Force).Msg("Provisioning MariaDB")
	var output strings.Builder

	// If force flag is set, remove existing credentials to regenerate them
	if p.Force {
		output.WriteString("=== Force mode: Regenerating credentials ===\n")
		configDir := "/opt/sitekit/config"
		os.Remove(configDir + "/.mysql_root")
		os.Remove(configDir + "/.mysql_sitekit")
	}
...
func (e *Executor) handleProvisionMySQL(ctx context.Context, payload json.RawMessage) comm.JobResult {
	...
	log.Info().Int64("step_id", p.StepID).Str("version", p.Version).Bool("force", p.Force).Msg("Provisioning MySQL")
	var output strings.Builder
	...
	if p.Force {
		output.WriteString("=== Force mode: Regenerating credentials ===\n")
		configDir := "/opt/sitekit/config"
		os.Remove(configDir + "/.mysql8_root")
		os.Remove(configDir + "/.mysql8_sitekit")
	}
...
func (e *Executor) handleProvisionPostgreSQL(ctx context.Context, payload json.RawMessage) comm.JobResult {
	...
	log.Info().Int64("step_id", p.StepID).Str("version", p.Version).Bool("force", p.Force).Msg("Provisioning PostgreSQL")
	var output strings.Builder
	...
	if p.Force {
		output.WriteString("=== Force mode: Regenerating credentials ===\n")
		configDir := "/opt/sitekit/config"
		os.Remove(configDir + "/.pgsql_sitekit")
	}
...
// Uses sudo to leverage unix_socket authentication for MariaDB/MySQL
func (e *Executor) runMySQLCommandNoAuth(ctx context.Context, query string) (string, int, error) {
	// First try mariadb command (for MariaDB installations)
	if _, err := os.Stat("/usr/bin/mariadb"); err == nil {
		return e.RunCommandWithExitCode(ctx, "mariadb", "-e", query)
	}

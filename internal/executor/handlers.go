...
func (e *Executor) handleCreateWebApp(ctx context.Context, payload json.RawMessage) comm.JobResult {
	...
	// Setup Node.js app if nodejs type
	if p.AppType == "nodejs" {
		// Create Node.js specific log directory
		nodeLogDir := filepath.Join(p.RootPath, "logs")
		if err := os.MkdirAll(nodeLogDir, 0755); err != nil {
			return comm.JobResult{Success: false, Error: fmt.Sprintf("failed to create node log dir: %v", err)}
		}

		// Create start.sh script for easy manual starting
...
if [ -s "/usr/local/nvm/nvm.sh" ]; then
    ...
    nvm use %s 2>/dev/null || true
fi

# Start the application
exec %s
`, p.Domain, p.RootPath, p.NodePort, p.Username, p.NodeVersion, startCmd)

		startScriptPath := filepath.Join(p.RootPath, "start.sh")
		if err := os.WriteFile(startScriptPath, []byte(startScript), 0755); err != nil {
			output.WriteString(fmt.Sprintf("Warning: failed to create start.sh: %v\n", err))
		} else {
			output.WriteString("Created start.sh script\n")
		}

		// Write supervisor config if provided
		if p.SupervisorConfig != "" {
			supervisorPath := fmt.Sprintf("/etc/supervisor/conf.d/nodejs-%s.conf", p.AppID)
			if err := os.WriteFile(supervisorPath, []byte(p.SupervisorConfig), 0644); err != nil {
				return comm.JobResult{Success: false, Error: fmt.Sprintf("failed to write supervisor config: %v", err)}
			}
			output.WriteString("Supervisor config installed\n")

			// Reload supervisor to pick up new config
			e.RunCommand(ctx, "supervisorctl", "reread")
			e.RunCommand(ctx, "supervisorctl", "update")
			output.WriteString("Supervisor reloaded\n")
		}

		// Create default index.html for Node.js app placeholder
		indexPath := filepath.Join(p.RootPath, "current", "index.html")
		if _, err := os.Stat(indexPath); os.IsNotExist(err) {
...
<body>
<h1>Welcome to %s</h1>
<p>Your Node.js application is not yet deployed.</p>
<p>Port: %d</p>
</body>
</html>`, p.Domain, p.Domain, p.NodePort)
			if err := os.WriteFile(indexPath, []byte(defaultIndex), 0644); err != nil {
				output.WriteString(fmt.Sprintf("Warning: failed to create default index.html: %v\n", err))
			} else {
				output.WriteString("Created placeholder index.html\n")
			}
		}
		...
		log.Info().
			Str("app_id", p.AppID).
			Int("port", p.NodePort).
			Str("node_version", p.NodeVersion).
			Msg("Node.js app setup complete")
	}

	// Setup static site if static type
	if p.AppType == "static" {
		// Create default index.html for static site placeholder
		indexPath := filepath.Join(p.RootPath, "current", p.PublicPath, "index.html")
		if _, err := os.Stat(indexPath); os.IsNotExist(err) {
			defaultIndex := fmt.Sprintf(`<!DOCTYPE html>
<html>
<head><title>%s</title></head>
<body>
<h1>Welcome to %s</h1>
<p>Your static site is ready for deployment.</p>
</body>
</html>`, p.Domain, p.Domain)
			if err := os.WriteFile(indexPath, []byte(defaultIndex), 0644); err != nil {
				output.WriteString(fmt.Sprintf("Warning: failed to create default index.html: %v\n", err))
			} else {
				output.WriteString("Created placeholder index.html\n")
...
func (e *Executor) getMySQLArgs(query string) []string {
	...
	// Fallback to reading sitekit password from file (preferred - has full privileges)
	data, err := os.ReadFile("/opt/sitekit/config/.mysql_sitekit")
	if err == nil {
		password := strings.TrimSpace(string(data))
		if password != "" {
			return []string{"-u", "sitekit", "-p" + password, "-e", query}
		}
	}
	// Last resort: try root password (may not work on unix_socket-only systems)
...
if [ -s "$NVM_DIR/nvm.sh" ]; then
    ...
    nvm use ` + p.NodeVersion + ` 2>/dev/null || true
fi

# Set Node.js environment
export NODE_ENV=production
export HOME=/home/` + p.Username + `
...
// handleTestDatabaseConnection tests database connectivity and returns health status
func (e *Executor) handleTestDatabaseConnection(ctx context.Context, payload json.RawMessage) comm.JobResult {
	var p struct {
		ServiceType string `json:"service_type"`
		ServiceID   string `json:"service_id"`
	}
	...
	log.Info().Str("service_type", p.ServiceType).Str("service_id", p.ServiceID).Msg("Testing database connection")

	var output strings.Builder
	output.WriteString(fmt.Sprintf("Testing %s connection...\n", p.ServiceType))

	switch p.ServiceType {
	case "mysql", "mariadb":
		return e.testMySQLConnection(ctx, &output)
	case "postgresql":
		return e.testPostgreSQLConnection(ctx, &output)
	default:
		return comm.JobResult{
			Success: false,
			Output:  output.String(),
			Error:   fmt.Sprintf("Unknown database type: %s", p.ServiceType),
		}
	}
}

func (e *Executor) testMySQLConnection(ctx context.Context, output *strings.Builder) comm.JobResult {
	...
	if password == "" {
		...
		if _, err := os.Stat("/usr/bin/mariadb"); err == nil {
			cmd = exec.CommandContext(ctx, "mariadb", "-e", "SELECT 'Connection OK' as status")
		} else {
			cmd = exec.CommandContext(ctx, "mysql", "-e", "SELECT 'Connection OK' as status")
		}
		...
		output.WriteString("\nConnection successful (socket auth)\n")
		return comm.JobResult{Success: true, Output: output.String()}
	}

	// Create temp defaults file
	tmpFile, err := os.CreateTemp("", "mysql-test-*.cnf")
	if err != nil {
		return comm.JobResult{Success: false, Error: "Failed to create temp file"}
	}
	defer os.Remove(tmpFile.Name())
	...
	out, _, err := e.RunCommandWithExitCode(ctx, cmdName, fmt.Sprintf("--defaults-extra-file=%s", tmpFile.Name()), "-e", "SELECT 'Connection OK' as status")
	output.WriteString(out)
...
func (e *Executor) testPostgreSQLConnection(ctx context.Context, output *strings.Builder) comm.JobResult {
	configDir := "/opt/sitekit/config"
	var password string
	...
	var cmd *exec.Cmd
	if password != "" {
		cmd = exec.CommandContext(ctx, "psql", "-U", "sitekit", "-h", "localhost", "-c", "SELECT 'Connection OK' as status")
		cmd.Env = append(os.Environ(), "PGPASSWORD="+password)
	} else {
		// Try peer auth
		cmd = exec.CommandContext(ctx, "sudo", "-u", "postgres", "psql", "-c", "SELECT 'Connection OK' as status")
	}
